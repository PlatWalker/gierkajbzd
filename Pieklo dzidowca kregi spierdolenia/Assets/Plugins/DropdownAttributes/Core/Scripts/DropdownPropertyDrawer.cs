        ...
        try
        {
            ...
            if (ilist.Count > 0)
                ...
                {//Use SystemObject.ToString()
                    ...
                    if (newSelectedID != SelectedID || !SystemObjectList.Contains(obj))
                    {//changed
                        SelectedID = newSelectedID;
                        ...
                        if (dropdownAttribute.ActionToPerformOnValueChange is not null)
                        {
                            BaseMaster.GetType().GetMethod(dropdownAttribute.ActionToPerformOnValueChange)?.Invoke(BaseMaster, Array.Empty<object>());
                        }
                        EditorUtility.SetDirty(property.serializedObject.targetObject);//repaint
                    }
