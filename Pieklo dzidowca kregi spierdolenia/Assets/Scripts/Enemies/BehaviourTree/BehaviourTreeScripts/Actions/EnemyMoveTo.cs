...
namespace jbzd.Enemies.BehaviourTree.Scripts.Actions
{
    ...
    public class EnemyMoveTo : ActionNode
    {
        ...
        protected override State OnUpdate() {
            if (!blackboard.moveEnemy) return State.Success;
        ...
        {
            var playerPosition = blackboard.EnemyData.MainCharacterTransform.position;
            var distanceToPlayer = Vector3.Distance(context.transform.position, playerPosition);
            
            if (distanceToPlayer > blackboard.stopDistance)
            {
                blackboard.NavAgent.SetDestination(playerPosition);
            }
            else
            {
                blackboard.NavAgent.ResetPath();
            }
        }
